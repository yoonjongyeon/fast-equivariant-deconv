...
class SingleSubjectdMRI(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.loading_method == 'h5':
            max_patch_data = torch.from_numpy(self.subject.h5_file[self.subject.data_path][lower[0]:upper[0], lower[1]:upper[1], lower[2]:upper[2]].copy())
        elif self.loading_method == 'memmap':
            max_patch_data = torch.from_numpy(self.subject.memmap[lower[0]:upper[0], lower[1]:upper[1], lower[2]:upper[2]].copy())
        elif self.loading_method == 'nibabel':
            normalization = self.subject.response_function.norm
            max_patch_data = torch.from_numpy(self.subject.image.image.dataobj[lower[0]:upper[0]][:, lower[1]:upper[1]][:, :, lower[2]:upper[2]].copy()) / normalization
        elif self.loading_method == 'numpy':
            max_patch_data = torch.from_numpy(self.subject.image.image[lower[0]:upper[0], lower[1]:upper[1], lower[2]:upper[2]].copy())
...
class MultiSubjectdMRI(Dataset):
    def __init__(self, subject_list, patch_size=1, concatenate=False, verbose=False):
        ...
        # bvals_input: Gradient mask applied b-value, bvals_output: Non-masked b-value
        # If no gradient mask is given (Null), bvals_input & bvals_output are same
        ...
        # Get input/output bvals mapping per subject (save index of bvals in each subject)
        ...
        # Get input max V and C (V: # b_vector, C: # SHC)
        ...
        # Get number of voxels in all masks (across all subjects)
    ...
    def __getitem__(self, index):
        ...
        vox = np.stack([x, y, z])   # center voxels
        # Get patch coordinates and # of bvals
        ...
        # Load data, depending on loading method
        ## max_patch_data: patched dwi data
        if self.loading_method == 'h5':
            max_patch_data = torch.from_numpy(subject.h5_file[subject.data_path][lower[0]:upper[0], lower[1]:upper[1], lower[2]:upper[2]].copy())
        elif self.loading_method == 'memmap':
            max_patch_data = torch.from_numpy(subject.memmap[lower[0]:upper[0], lower[1]:upper[1], lower[2]:upper[2]].copy())
        elif self.loading_method == 'nibabel':
            normalization = self.group_response_functions.response_function_list[subject_index].norm
            max_patch_data = torch.from_numpy(subject.image.image.dataobj[lower[0]:upper[0]][:, lower[1]:upper[1]][:, :, lower[2]:upper[2]].copy()) / normalization
        elif self.loading_method == 'numpy':
            max_patch_data = torch.from_numpy(subject.image.image[lower[0]:upper[0], lower[1]:upper[1], lower[2]:upper[2]].copy())
        ...
        if self.concatenate: # not true always in our setting
            input_features = torch.flatten(input_features, start_dim=0, end_dim=2) # P*P*P x V
            input_features = input_features[:, :, None, None, None] # P*P*P x V x 1 x 1 x 1
        else:
            input_features = input_features.permute(3, 0, 1, 2)[None] # 1 x V x P x P x P
        ...
        batch = {'input_features': input_features, 'output_features': output_features, 'output_b0': output_b0, 'output_mask': output_mask, 'input_signal_to_shc': input_signal_to_shc, 'output_shc_to_signal': output_shc_to_signal, 'vox': vox}

        if self.has_fodf:
            if self.n_fodf_anisotropic>0:
                max_patch_data_fodf_anisotropic = torch.from_numpy(subject.fodfs.fodf_anisotropic[:, lower[0]:upper[0], lower[1]:upper[1], lower[2]:upper[2]].copy())
            ...
            if self.n_fodf_isotropic>0:
                max_patch_data_fodf_isotropic = torch.from_numpy(subject.fodfs.fodf_isotropic[:, lower[0]:upper[0], lower[1]:upper[1], lower[2]:upper[2]].copy())
