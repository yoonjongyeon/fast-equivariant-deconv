...
def _scipy_ckdtree(features, k):
    distance  = np.sqrt(np.sum((features[:, None] - features[None, :])**2, axis=-1))
    indx = np.arange(distance.shape[0])[None].repeat(distance.shape[0], axis=0)
    sel = distance > 2 - 1e-5
    assert np.sum(np.sum(sel, axis=-1) != 1) == 0
    match_sel = indx[sel]

    argsort_index = np.argsort(distance, axis=-1)
    sort_distance = np.sort(distance, axis=-1)
    radius_max_per_vertice = sort_distance[:, k]
    mask = sort_distance < radius_max_per_vertice[:, None] + 1e-5
    dist = []
    neigh = []
    for v in range(len(mask)):
        dist.append(sort_distance[v, mask[v]])
        neigh.append(argsort_index[v, mask[v]])

    for i in range(len(match_sel)):
        n_i = neigh[i]
        n_si = match_sel[neigh[match_sel[i]]]

        d_i = dist[i]
        d_si = dist[match_sel[i]]

        assert len(n_i) == len(n_si)
        assert len(d_i) == len(d_si)
        assert len(n_i)>=k+1
        assert set(n_i) == set(n_si)
        assert np.sum(d_i - d_si > 1e-8) == 0
    return neigh, dist
...
class NNGraph(Graph):
    ...
    def __init__(self, features, standardize=False,
        ...
        neighbors, distances = function(features, k)
