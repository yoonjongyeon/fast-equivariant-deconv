...
scoring_data= #DOWNLOAD SCORING DATA ~/scoring_data_Renauld2023
...
echo '------------- SEGMENTATION ------------'
/ext3/scilpy2/scripts/scil_score_tractogram.py $tractogram $config_file_segmentation $out_dir --no_empty \
...
if [ "$CC_files" != '' ]
then
    /ext3/scilpy2/scripts/scil_tractogram_math.py lazy_concatenate $CC_files $out_dir/segmented_VB/CC_VS.trk;
fi
echo '------------- Merging ICP left sub-bundles ------------'
ICP_left_files=$(ls $out_dir/segmented_VB/ICP_left* 2> /dev/null)
if [ "$ICP_left_files" != '' ]
then
    /ext3/scilpy2/scripts/scil_tractogram_math.py lazy_concatenate $ICP_left_files $out_dir/segmented_VB/ICP_left_VS.trk;
fi
echo '------------- Merging ICP right sub-bundles ------------'
ICP_right_files=$(ls $out_dir/segmented_VB/ICP_right* 2> /dev/null)
if [ "$ICP_right_files" != '' ]
then
    /ext3/scilpy2/scripts/scil_tractogram_math.py lazy_concatenate $ICP_right_files  $out_dir/segmented_VB/ICP_right_VS.trk;
fi

echo '------------- FINAL SCORING ------------'
/ext3/scilpy2/scripts/scil_bundle_score_many_bundles_one_tractogram.py -v $config_file_tractometry $out_dir \
