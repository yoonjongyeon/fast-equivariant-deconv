...
class GraphCNNUnet(nn.Module):
    """GCNN Autoencoder.
    """
    def __init__(self, in_channels, out_channels, filter_start, block_depth, in_depth, kernel_sizeSph, kernel_sizeSpa, poolings, laps, conv_name, isoSpa, keepSphericalDim, patch_size_list, vec, nvec_out=None):
        """Initialization.
        Args:
        ...
        print(f'Unet in channels: {in_channels} - out channels {out_channels} - nvecs out {nvec_out}')
        self.encoder = Encoder(in_channels, filter_start, block_depth, kernel_sizeSph, kernel_sizeSpa, poolings, laps, conv_name, isoSpa, patch_size_list, vec)
        self.decoder = Decoder(out_channels, filter_start, in_depth, kernel_sizeSph, kernel_sizeSpa, poolings, laps, conv_name, isoSpa, patch_size_list, keepSphericalDim, vec, nvec_out)
...
class Encoder(nn.Module):
    """GCNN Encoder.
    """
    def __init__(self, in_channels, filter_start, block_depth, kernel_sizeSph, kernel_sizeSpa, poolings, laps, conv_name, isoSpa, patch_size_list, vec):
        """Initialization.
        Args:
            in_channels (int): Number of input channel
            filter_start (int): Number of feature channel after first convolution. Then, multiplied by 2 after every poolig
        ...
        D = len(laps)
        print(f'Create Encoder with {D-1} levels and {block_depth} convolution per level')
        assert D > 1 # Otherwise there is no encoding/decoding to perform
        self.enc_blocks = [Block([in_channels] + [filter_start]*block_depth, laps[-1], kernel_sizeSph, kernel_sizeSpa, conv_name=conv_name, isoSpa=isoSpa, patch_size=patch_size_list[-1], vec=[vec[-1]] + [vec[-1]]*(block_depth-1) + [vec[-2]])]
        self.enc_blocks += [Block([(2**i)*filter_start] + [(2**(i+1))*filter_start]*block_depth, laps[-i-2], kernel_sizeSph, kernel_sizeSpa, conv_name=conv_name, isoSpa=isoSpa, patch_size=patch_size_list[-i-2], vec=[vec[-i-2]] + [vec[-i-2]]*(block_depth-1) + [vec[-i-3]]) for i in range(D-2)]
...
class Decoder(nn.Module):
    """GCNN Decoder.
    """
    def __init__(self, out_channels, filter_start, in_depth, kernel_sizeSph, kernel_sizeSpa, poolings, laps, conv_name, isoSpa, patch_size_list, keepSphericalDim, vec, n_vec):
        """Initialization.
        Args:
            out_channels (int): Number of output channel
            filter_start (int): Number of feature channel after first convolution. Then, multiplied by 2 after every poolig
        ...
        D = len(laps)
        print(f'Create Decoder with {D} levels and {in_depth+1} convolution per level and a head with 1 convolution')
        assert D > 1 # Otherwise there is no encoding/decoding to perform
        self.dec_blocks = [Block([(2**(D-2))*filter_start] + [(2**(D-1))*filter_start]*in_depth + [(2**(D-2))*filter_start], laps[0], kernel_sizeSph, kernel_sizeSpa, conv_name=conv_name, isoSpa=isoSpa, patch_size=patch_size_list[0], vec=[vec[0]] + [vec[0]]*in_depth + [vec[0]])]
        self.dec_blocks += [Block([(2**(D-i))*filter_start] + [(2**(D-i-1))*filter_start]*in_depth + [(2**(D-i-2))*filter_start], laps[i], kernel_sizeSph, kernel_sizeSpa, conv_name=conv_name, isoSpa=isoSpa, patch_size=patch_size_list[i], vec=[vec[i]] + [vec[i]]*in_depth + [vec[i]]) for i in range(1, D-1)]
        self.dec_blocks += [Block([2*filter_start] + [filter_start]*in_depth + [filter_start], laps[-1], kernel_sizeSph, kernel_sizeSpa, conv_name=conv_name, isoSpa=isoSpa, patch_size=patch_size_list[-1], vec=[vec[-1]] + [vec[-1]]*in_depth + [vec[-1]])]
        self.dec_blocks = nn.ModuleList(self.dec_blocks)
        self.head = BlockHead([filter_start, out_channels], laps[-1], 1, 1, conv_name, isoSpa, patch_size_list[-1], keepSphericalDim, [vec[-1], vec[-1]], n_vec)
